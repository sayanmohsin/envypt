...
use open_envault::crypto::keys::{self, Identity};
...
fn decrypt_sops_single_recipient() {
    ...
    let out = open_envault::crypto::store::decrypt(&encrypted, &[dev_identity()]).unwrap();
...
fn decrypt_sops_multi_recipient_with_either_key() {
    ...
    let plaintext = read("dev.env");
    assert_eq!(
        open_envault::crypto::store::decrypt(&encrypted, &[dev_identity()]).unwrap(),
        plaintext
    );
    assert_eq!(
        open_envault::crypto::store::decrypt(&encrypted, &[ci_identity()]).unwrap(),
        plaintext
    );
}
...
fn wrong_key_is_rejected() {
    let encrypted = read("dev.env.enc");
    let err = open_envault::crypto::store::decrypt(&encrypted, &[ci_identity()]).unwrap_err();
    let text = format!("{err:#}");
    assert!(
...
fn tampered_value_is_rejected() {
    ...
    assert!(open_envault::crypto::store::decrypt(&encrypted, &[dev_identity()]).is_err());
}
...
fn plaintext_features_roundtrip_through_our_encrypt() {
    ...
    let encrypted = open_envault::crypto::store::encrypt(&plaintext, &[dev_recipient]).unwrap();
    ...
    let decrypted = open_envault::crypto::store::decrypt(&encrypted, &[dev_identity()]).unwrap();
    assert_eq!(decrypted, plaintext);
}

#[test]
fn list_recipients_matches_metadata() {
    let dev = open_envault::crypto::store::list_recipients(&read("dev.env.enc")).unwrap();
    assert_eq!(dev.len(), 1);
    let multi = open_envault::crypto::store::list_recipients(&read("multi.env.enc")).unwrap();
...
fn empty_and_reserved_keys_fail_closed() {
    ...
    let err = open_envault::crypto::store::encrypt("A=1", &[]).unwrap_err();
    assert!(format!("{err:#}").contains("no age recipients"));

    let err = open_envault::crypto::store::encrypt("sops_evil=1", std::slice::from_ref(&recipient))
        .unwrap_err();
    assert!(format!("{err:#}").contains("reserved"));

    let err = open_envault::crypto::store::encrypt("A=1\nnot-a-key\n", &[recipient]).unwrap_err();
...
fn official_sops_can_decrypt_our_output() {
    ...
    let plaintext = read("dev.env");
    let recipient = read_recipient_of("age-identity-dev.txt");
    let encrypted = open_envault::crypto::store::encrypt(&plaintext, &[recipient]).unwrap();
