//! Public API for openenvcrypt.
//!
//! Consumers (e.g. the Arqen framework) depend on this facade crate rather
//! than on the internal `openenvcrypt-core`/`-crypto` crates. Values returned
//! here exist only in process memory; never log or persist them.
//!
//! ```no_run
//! if let Ok(values) = openenvcrypt::load_environment("dev") {
...
pub use openenvcrypt_core as core;
pub use openenvcrypt_crypto as crypto;
pub use openenvcrypt_output as output;
pub use openenvcrypt_runtime as runtime;
pub use openenvcrypt_schema as schema;
...
/// (see [`runtime::exec`] and `openenvcrypt-core` docs). This function never
