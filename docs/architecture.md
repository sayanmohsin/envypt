...
- **Storage format is SOPS-compatible age.** Files produced by
  `openenvcrypt` are plain SOPS files (age recipients + AES-256-GCM values +
  a SOPS metadata block). Any official `sops`/`age` tooling can decrypt them,
  and `openenvcrypt` decrypts files produced by official `sops`.
...
|---|---|
| `openenvcrypt` | Facade library crate exposing the public Rust API (`load_environment`, `check`, `doctor`, `diff`, `exec`). Consumed by Arqen. |
| `openenvcrypt-cli` | The `openenvcrypt` binary: clap surface, command orchestration, human/JSON output. |
| `openenvcrypt-core` | Project config (`openenvcrypt.yaml`), upward discovery, environment profiles, key-source resolution, atomic file IO. |
| `openenvcrypt-crypto` | Native SOPS-over-age: data-key generation, age key wrapping per recipient, AES-256-GCM value encryption, SOPS metadata + MAC, key generation/parsing, permission enforcement. |
| `openenvcrypt-schema` | Schema types, per-type validation, dotenv parse/write, `.env.example` generation. |
| `openenvcrypt-runtime` | Child process execution, environment merging, signal forwarding, exit-code propagation. |
| `openenvcrypt-output` | Redaction, one-way fingerprinting, JSON envelopes, shared error/exit-code contract. |

## Project layout on disk

```text
openenvcrypt.yaml        project config (environments, recipients, key files)
...
3. Profile `key_file` from `openenvcrypt.yaml`.
4. Developer default:
   `$XDG_CONFIG_HOME|~/.config/openenvcrypt/keys/<environment>.txt`.
