...
- `envypt.yaml`, `config/env.schema.yaml`, `.env.example`
...
- Keys are `age` x25519 (`age-keygen`). Files are `0600`, outside the repo: `~/.config/envypt/keys/<env>.txt` or `XDG_CONFIG_HOME`.
- CI uses `ENVYPT_AGE_KEY` / `SOPS_AGE_KEY` (private key text) or `*_KEY_FILE`.
- Separate recipients per environment (`dev`, `prod`, `ci`).

## Runtime

- `envypt exec` decrypts SOPS age files with `age` + `AES-256-GCM` (32-byte nonce), verifies the `sops_mac` (`HMAC-SHA512` over the plaintext tree), and injects values only into the child’s environment (`execve`).
...
- `sops` compatibility means any `sops`-aware tooling can audit the files, but `envypt` never shells out to `sops` at runtime.
