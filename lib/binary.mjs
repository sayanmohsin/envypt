...
export function binaryPath() {
  ...
  if (!dir) {
    throw new Error(`oenv: unsupported platform ${key}`);
  }
  const exe = process.platform === "win32" ? "oenv.exe" : "oenv";
  ...
  throw new Error(`oenv: binary not found for ${key}. Tried ${prebuild}`);
}
